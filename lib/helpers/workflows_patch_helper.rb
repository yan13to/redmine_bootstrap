...
module Helpers
  module WorkflowsPatchHelper
    def options_for_workflow_select(name, objects, selected, options={})
      ...
      page_select_tag name, option_tags, {:multiple => multiple}.merge(options)
    end
    ...
    def field_permission_tag(permissions, status, field, roles)
      ...
      page_select_tag("permissions[#{status.id}][#{name}]", options_for_select(options, selected), html_options)
    end
    ...
    def transition_tag(transition_count, old_status, new_status, name)
      ...
      if old_status == new_status
        page_check_box_tag(tag_name, "1", true,
        ...
        page_check_box_tag(tag_name, "1", w != 0,
...
WorkflowsHelper.send(:prepend, Helpers::WorkflowsPatchHelper)